...
1. Bajen el archivo datos_alumnos.xls de la carpeta de la clase de hoy, ponganlo en el Escritorio, abranlo en _Excel_ y conviertanlo a formato .xls.

2. Abran R y lean el archivo usando la funcion **read.csv()** (leyendo encabezados [_header = T_]) y almacenando la base de datos con el nombre _datos._
...
Tenemos informacion que necesariamente necesita la funcion:

_x_ 

e informacion adicional:
...
¿Que pasa entonces cuando cambiamos el valor de trim? ¿Digamos que queremos quitar un valor de cada extremo de datos$Talla.zapatos, o sea, la talla mas chica y la talla mas grande?

¿Cuantos datos tenemos de datos$Talla.zapatos? Usamos la funcion **length()** para saber la longitud de un vector:
...
Combinando funciones podemos ponerlo quitar la talla mas chica y la mas grande sin fijarnos en cuantos datos tenemos en total:
```
...
¿Que pasa cuando tratabamos calcular la moda de datos$Edad? usando la funcion **mode()**
...
Pero como el formato no me gusta, usamos **frecs = as.data.frame(table(x))** y despues les damos nombre a las columnas con la funcion **colnames(frecs) = c("Variable","Frecuencia.Absoluta")**.
...
colnames(frecs) = c("Frecuencia.respiratoria.ejercicio.fisico","Frecuencia.absoluta")
frecs
```

**Frecuencia relativa**

Basta con que dividamos las frecuencias absolutas entre **sum(frecs$Frecuencia.absoluta)**:
```
frec.rel = frecs$Frecuencia.absoluta/sum(frecs$Frecuencia.absoluta)
```

Para agregar la _frec.rel_ a la tabla _frecs_, usamos la funcion **cbind()** que agrega columnas a una base de datos:
```
frecs = cbind(frecs,frec.rel)
frecs
```
...
```
frec.acum = cumsum(frecs$Frecuencia.absoluta)
frecs = cbind(frecs,frec.acum)
frecs
```
...
_ACTIVIDAD: escojan una variable categorica y una continua y hagan sus graficas correspondientes._

_RETO: calculen la desviacion estandar de datos$Edad sin usar la funcion **sd()**._
